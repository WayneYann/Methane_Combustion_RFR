function [tout, yout, Eigenmatrix] = ode15s_JPJ(ode,tspan,y0,options,varargin)
...
if Jconstant 
  if isempty(Jac) % use odenumjac
    ...
    nfevals = nfevals + nF;
    npds = npds + 1;
      %Josh Lansford: Altered Code by
              Eigenvalues = eig(dfdy);
              numsmall = 0;
              NumLargeimag = 0;
              Eigenlength = length(Eigenvalues);
              normEigen = zeros(Eigenlength,1);
              realEigen = zeros(Eigenlength,1);
              imagEigen = zeros(Eigenlength,1);
              for i = 1:Eigenlength
                  normEigen(i) = norm(Eigenvalues(i));
                  realEigen(i) = real(Eigenvalues(i));
                  imagEigen(i) = imag(Eigenvalues(i));
                  if abs(realEigen(i)) < 1
                      numsmall = numsmall+1;
                      if abs(imagEigen(i)) > 10*abs(realEigen(i));
                          NumLargeimag = NumLargeimag+1;
                      end
                  end
              end
              maxEigen = max(normEigen);
              minEigen = min(normEigen);
              maxReal = max(realEigen);
              numlarge = Eigenlength - numsmall;
              Eigenmatrix(jacobicount,:) = [maxReal,maxEigen, minEigen, numlarge, numsmall, NumLargeimag, cond(dfdy)];
              jacobicount = jacobicount+1;
...
%Edited by Josh
tout = solver_output{1};
yout = solver_output{2};
